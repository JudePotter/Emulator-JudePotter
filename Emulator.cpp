...
#define STUDENT_NUMBER    "21004175"
...
void set_flag_n(BYTE inReg) // Setting flag, from blackboard
{
	BYTE reg;
	reg = inReg;
...
void set_flag_n(WORD inReg) // Setting flag, overloading to complete LDZ 16bit 
{
	WORD reg;
	reg = inReg;
...
void set_flag_z(WORD inReg) // 16bit set
{
	WORD reg;
	reg = inReg;
	if (reg == 0) // zero set
...
void Group_1(BYTE opcode){
	...
	case 0x00: // JR Jump Subroutine
		LB = fetch();
		HB = fetch();
		address = ((WORD)HB << 8) + (WORD)LB;
		push((BYTE)((ProgramCounter >> 8) & 0xFF)); // Pushes High Byte 
		push((BYTE)(ProgramCounter & 0xFF)); // Pushes Low Byte
		ProgramCounter = address;
		break;
	
	case 0x86: // RTS Return Subroutine
		LB = pop(); // LB first becuase it is a stack, last in first out 
		HB = pop();
		address = ((WORD)HB << 8) + (WORD)LB; 
		ProgramCounter = address;
		break;

	case 0xE7: // BRA relative addressing
		offset = (WORD)fetch();
		if((offset & 0x80) != 0) 
		{
			offset = offset + 0xFF00; // need to sign extend
		}
		address = ProgramCounter + offset;
		ProgramCounter = address;
		break;
		
		// Branches but testing flags, use if statement only if Carry was set or clear or pos or negative number 
		// if a = b, compare a = b 
		// same as above code but use if statement after "address = ProgramCounter + offset;"
		// if condition is correct then do the jump 01:31:45
	...
	case 0x9A: // IOR A,F
		ior(REGISTER_A, REGISTER_F);
		break;

	case 0xAA: // IOR B,C
		ior(REGISTER_B, REGISTER_C);
		break;

	case 0xBA: // IOR B,D
		ior(REGISTER_B, REGISTER_D);
		break;

	case 0xCA: // IOR B,E
		ior(REGISTER_B, REGISTER_E);
		break;

	case 0xDA: // IOR B,F
		ior(REGISTER_B, REGISTER_F);
		break;
